...
function Schedule:schedule(cb, dt, delay, tag)
	...
	self.scheduleCbMap[tag] = {cb = cb, dt = dt, delay = delay or 0}  --已经存在的直接覆盖
...
function Schedule:onScheduleUpdate_(target, delta)
	if self.scheduleCbOrder == nil then
		local sorted = {} -- 保证tag是递增的
		sorted = table.keys(self.scheduleCbMap)
		table.sort(sorted)
		self.scheduleCbOrder = sorted
	end

	local over = 0
	for _, tag in ipairs(self.scheduleCbOrder) do
		local v = self.scheduleCbMap[tag]
		if v then -- v.cb可能导致后续的schedule被清理
			if v.delay < 0 then --确保要在下一帧才开始执行v.cb,因为有些cdx对象初始化后当帧拿到的数据还没刷新
				local ret = v.cb(v.dt)
				if ret == false then  --函数内部返回false 代表此schedule结束
					self.scheduleCbMap[tag] = nil
					over = over + 1
				else
