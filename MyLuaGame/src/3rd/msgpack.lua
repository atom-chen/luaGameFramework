...
unpackers['any'] = function (c)
    ...
    if DEBUG_MSGPACK then 
        ret = unpackers[types_map[val]](c, val)
        print('msgpack unpack:', types_map[val], ret)
        return ret
    end
...
unpackers['double'] = function (c)
    ...
    elseif expo == 0x7FF then
        ...
        -- modify by huangwei. 2014/7/24
        n = sign * ldexp(1.0 + mant / two52, expo - 0x3FF)
    end
    c.i = i+8
    return n
end
...
function m.unpack (s)
    ...
    if cursor.i < cursor.j then
        error (strfmt("extra bytes %d %d", cursor.i, cursor.j))
    end
    return data
end
...
if DEBUG_MSGPACK then
    ...
    print ('msgpack: NUMBER_INTEGRAL', NUMBER_INTEGRAL)
