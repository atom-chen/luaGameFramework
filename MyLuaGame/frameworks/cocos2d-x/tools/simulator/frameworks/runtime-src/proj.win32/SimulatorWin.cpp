...
void SimulatorWin::onOpenFile(const std::string &filePath)
{
    ...
    if (stringEndWith(entry, "config.json") || stringEndWith(entry, ".csb") || stringEndWith(entry, ".csd"))
    {
        ...
        {
			auto parser = ConfigParser::getInstance();
			parser->readConfig(entry);

			// set information
			_project.setConsolePort(parser->getConsolePort());
			_project.setFileUploadPort(parser->getUploadPort());
			_project.setFrameSize(parser->getInitViewSize());
			if (parser->isLanscape())
			{
			...
			_project.setScriptFile(parser->getEntryFile());
        }
