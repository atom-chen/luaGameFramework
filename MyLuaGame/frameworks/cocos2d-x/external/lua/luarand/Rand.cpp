...
double frandomMT( unsigned int *state, unsigned int *&next, int &left );
...
double frandomMT( unsigned int *state, unsigned int *&next, int &left )
{
	return ( ( double ) randomMT(state, next, left) / 4294967296.0 );
}
...
double RakNetRandom::FrandomMT( void )
{
